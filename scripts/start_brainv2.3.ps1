<#
.SYNOPSIS
    Soma "Distributed Brain" Startup Orchestrator v2.3 (Robust)

.DESCRIPTION
    Launches the Soma ecosystem.
    Fixes 0x80070002 & ModuleNotFoundError by using temporary launcher scripts and setting PYTHONPATH.
#>
...
function Start-ServiceTab {
    param(
        ...
        [string]$Command,
        [string]$Color = "Cyan",
        [string]$RepoRoot = $REPO_ROOT # Pass repo root for PYTHONPATH
    )
    ...
    $TempScriptPath = "$env:TEMP\soma_launch_$($ServiceName -replace '[^a-zA-Z0-9]','').ps1"
...
Write-Host "🐍 PYTHONPATH set to include root." -ForegroundColor DarkGray
...
"@
    ...
    # 2. Launch the script in a new window/tab
    # We use powershell.exe -File ... which is the most robust way to launch code
    
    if (Get-Command wt.exe -ErrorAction SilentlyContinue) {
        # Launch in Windows Terminal
        Start-Process wt.exe -ArgumentList "new-tab", "--title", "$ServiceName", "-p", "PowerShell", "powershell.exe", "-NoExit", "-File", "$TempScriptPath"
    }
    else {
...
# We prefer the venv in the root
...
if (Test-Path $VenvPython) {
    ...
    # Use full paths to binaries to avoid ambiguity
    $PyCmd = "& '$VenvPython'"
...
# 1. memOS (Hands)
# We pass the command string. Note the escaping of quotes isn't needed as much now.
...
# 2. OmegaKG (Brain)
# Critical: This runs from $REPO_ROOT\OmegaKG but needs to see 'omega_kg' package
