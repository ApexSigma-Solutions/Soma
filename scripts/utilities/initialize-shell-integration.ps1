# -----------------------------------------------------------------------------
# Omega_KG Shell Integration - CORRECTED & UNIFIED (ApexSigma)
# -----------------------------------------------------------------------------
#
...
# FIXED: Point to the unified directory
$global:OMEGA_KG_PROJECT_ROOT = "D:\projects\OmegaKG"
...
function Set-OmegaKGEnvironment {
    ...
    # This legacy function now just ensures we point to the main root
    $global:OMEGA_KG_PROJECT_ROOT = "D:\projects\OmegaKG"
...
# Source the venv helper functions (setup-venv.ps1)
# FIXED: Point to Omega_KG_stable location
$venvScriptPath = Join-Path $global:OMEGA_KG_PROJECT_ROOT "Omega_KG_stable\scripts\maintenance\setup-venv.ps1"
        ...
        if ($global:OMEGA_KG_PROJECT_ROOT -and (Get-Location).Path -like "$global:OMEGA_KG_PROJECT_ROOT*") {
            ...
            if (-not (Test-Path "variable:global:OMEGA_KG_CHECK_COMPLETE")) {
                ...
                if (Get-Command activate-omega -ErrorAction SilentlyContinue) {
                    ...
                    Write-Host "[X] FAILURE: 'activate-omega' function not found. (Sourcing 'setup-venv.ps1' failed)" -ForegroundColor Red
                }
                
                # 2. RUN Pre-flight checks manually if needed
                # FIXED: Point to Omega_KG_stable location
                $checkScript = Join-Path (Join-Path $global:OMEGA_KG_PROJECT_ROOT "Omega_KG_stable\scripts\startup") "start-dev.ps1"
...
function omega-preflight {
    ...
    if (Test-Path $checkScript) {
        ...
        Write-Host "[!] WARNING: 'start-dev.ps1' not found at $checkScript" -ForegroundColor Yellow
    }
}
...
function prompt {
    ...
    # FIXED: Force ApexSigma prompt
    $projectName = if ($global:OMEGA_KG_PROJECT_ROOT) { "[ApexSigma] " } else { "" }
...
function omega-start {
    ...
    #>
    if (-not $global:OMEGA_KG_PROJECT_ROOT) {
        Write-Error "Not in an Omega_KG environment."
        return
    }
    & "$global:OMEGA_KG_PROJECT_ROOT\Omega_KG_stable\scripts\Start-OmegaServer.ps1" -Mode foreground
}
...
function omega-start-term {
    ...
    & "$global:OMEGA_KG_PROJECT_ROOT\Omega_KG_stable\scripts\Start-OmegaServer.ps1" -Mode terminal
}
...
function omega-start-bg {
    ...
    & "$global:OMEGA_KG_PROJECT_ROOT\Omega_KG_stable\scripts\Start-OmegaServer.ps1" -Mode background
}
...
function omega-stop {
    ...
    & "$global:OMEGA_KG_PROJECT_ROOT\Omega_KG_stable\scripts\Start-OmegaServer.ps1" -Mode stop
}
...
function omega-restart {
    ...
    & "$global:OMEGA_KG_PROJECT_ROOT\Omega_KG_stable\scripts\Start-OmegaServer.ps1" -Mode restart
}
...
function omega-status {
    ...
    & "$global:OMEGA_KG_PROJECT_ROOT\Omega_KG_stable\scripts\Start-OmegaServer.ps1" -Mode status
}
...
function omega-code {
    ...
    # FIXED: Always open main project
    $projectPath = "d:\projects\OmegaKG"
    & code $projectPath
}
