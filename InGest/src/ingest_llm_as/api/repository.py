...
async def ingest_python_repository(
    ...
    try:
        raw_record = RawIngestion(
            ...
            raw_metadata={
                ...
                "exclude_patterns": request.exclude_patterns[
                    :METADATA_LIST_TRUNCATION_LIMIT
                ],  # First 5 for brevity
            },
        ...
        logger.info(
            f"Raw repository ingestion persisted: {ingestion_id} ({request.source_path})"
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to persist raw repository ingestion data"
        ) from e
